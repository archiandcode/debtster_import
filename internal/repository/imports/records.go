...
func SoftDeleteImportRecord(ctx context.Context, m *mg.Mongo, id string) error {
    if m == nil || m.Database == nil {
    ...
    if oid, err := primitive.ObjectIDFromHex(id); err == nil {
        res, err := coll.UpdateOne(ctx, bson.M{"_id": oid}, bson.M{"$set": bson.M{"deleted_at": now, "updated_at": now}})
        if err == nil && res.MatchedCount > 0 {
            return nil
        }
    }

    res, err := coll.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": bson.M{"deleted_at": now, "updated_at": now}})
    if err != nil {
        return err
    }
    if res.MatchedCount == 0 {
        return fmt.Errorf("no import_record found with id %s", id)
    }
    return nil
}

func RestoreImportRecord(ctx context.Context, m *mg.Mongo, id string) error {
    if m == nil || m.Database == nil {
        return mongo.ErrClientDisconnected
    }
    coll := m.Database.Collection(ImportRecordsCollection)
    ...
    now := time.Now().UTC()
    if oid, err := primitive.ObjectIDFromHex(id); err == nil {
        res, err := coll.UpdateOne(ctx, bson.M{"_id": oid}, bson.M{"$unset": bson.M{"deleted_at": ""}, "$set": bson.M{"updated_at": now}})
        if err == nil && res.MatchedCount > 0 {
            return nil
        }
    }

    res, err := coll.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$unset": bson.M{"deleted_at": ""}, "$set": bson.M{"updated_at": now}})
    if err != nil {
        return err
    }
    if res.MatchedCount == 0 {
...
func ForceDeleteImportRecord(ctx context.Context, m *mg.Mongo, id string) error {
    if m == nil || m.Database == nil {
        return mongo.ErrClientDisconnected
    }
    coll := m.Database.Collection(ImportRecordsCollection)
...
func ListImportRecordItems(ctx context.Context, m *mg.Mongo, importRecordID string, limit, skip int64) ([]Item, int64, error) {
    if m == nil || m.Database == nil {
        return nil, 0, mongo.ErrClientDisconnected
    }
    coll := m.Database.Collection(ImportRecordItemsCollection)
    filter := bson.M{"import_record_id": importRecordID}
